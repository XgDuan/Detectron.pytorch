...
class GqaDataset(object):
    ...
    def _prepare_category(self):
        """
        Prepare categories. For training, we
        """
    ...
    def get_roidb(self, gt=False, proposal_file=None, min_proposal_size=2, proposal_limit=-1, crowd_filter_thresh=0):
        ...
        roidb = [{
            ...
            'id': idx
        } for idx, scene_graph in list(self.scenes.items())]  # debug mode
        for entry in roidb:
    ...
    def _add_gt_from_cache(self, roidb, cache_filepath):
        ...
        for entry, cached_entry in zip(roidb, cached_roidb):
            entry['boxes'] = np.append(entry['boxes'], cached_entry['boxes'], axis=0)
            # To match the original implementation:
            # entry['boxes'] = np.append(
            #     entry['boxes'], boxes.astype(np.int).astype(np.float), axis=0)
            entry['gt_classes'] = np.append(entry['gt_classes'], cached_entry['gt_classes'])
            entry['gt_overlaps'] = scipy.sparse.csr_matrix(cached_entry['gt_overlaps'])
            entry['is_crowd'] = np.append(entry['is_crowd'], cached_entry['is_crowd'])
            entry['box_to_gt_ind_map'] = np.append(
                entry['box_to_gt_ind_map'], cached_entry['box_to_gt_ind_map']
            )
